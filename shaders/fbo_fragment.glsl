...
vec3 applyColorGrading(vec3 color) {
    vec2 lutCoords = vec2(color.r, color.g);
    vec3 gradedColor = texture(colorGradingLUT, lutCoords).rgb;
    return mix(color, gradedColor, colorGradingIntensity);
}
...
void main() {
    ...
    // Apply color grading with smoothing
    vec3 gradedColor = applyColorGrading(mapped);
    mapped = mix(mapped, gradedColor, colorGradingIntensity);
